...
<body>
    ...
    <script>
        ...
        async function submitOrder(){await loadBalance();if(parseFloat(document.getElementById('balanceDisplay').textContent.replace('$',''))<price){alert('❌ رصيد غير كاف');return}const b=event.target;b.textContent='⏳';b.disabled=true;const r=await fetch(`/api/order?uid=${uid}&country=${document.getElementById('country').value}&service=${document.getElementById('service').value}&price=${price}`);const d=await r.json();if(d.ok){await loadBalance();alert('✅ تم تقديم طلبك! انتظر الموافقة 🔔')}else{alert('❌ '+d.error)}b.textContent='⚡ احصل على رقم الآن';b.disabled=false}
